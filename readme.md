...
| :----------------------- | :------------------------------------ | :--------------------------------------------------------------------------------------------------- | :------------------------------------------ | :--------------------------------------- |
| **MCU**                  |     microcontroller unit              | Integrovaný čip (CPU + RAM + Flash na jednom substrátu), deterministický běh bez OS nebo RTOS        | např. ESP32, PIC16LF1xxx, RP2040            | automobilova palubka                     |
...
*Vaše odpověď:*
`OCHRANA proti vysokotlakemu a vysokoteplotnimu blizkemu střiku`
...
| :---------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------ | :------------------------ | :----------------------------------- |
| **Doporučená platforma** *(MCU / PLC / iPC)*                                                          | **Programovatelné relé / kompaktní PLC** *(např. Siemens LOGO!, Eaton easyE4)*                                                                                                                                            | `MCU(STM32,ESP32)`              | `plc(SIEMENS LOGO!,SIEMENS S7-1200)`  | `ipc(beckhoff embedded pc)` |
| **Pořizovací cena HW na 1 kus** *(nízká < 500 Kč / střední 5–30 tis. Kč / vysoká > 50 tis. Kč)*       | **Střední** *(cca 3 500 – 6 000 Kč)*                                                                                                                                                                                      | `nizka`                         | `středni`                 | `vysoka`                              |
| **Primární programovací jazyk** *(C/C++/MicroPython vs. IEC 61131-3 ST/LAD vs. Python/C#/C++ pod OS)* | **FBD / LAD** *(grafické funkční bloky nebo liniové schéma dle IEC 61131-3)*                                                                                                                                              | `c/c++`                         | `FBD/LAD dle IEC 61131-3`    | `python/C/C++ pod OS`               |
| **Klíčový technický argument pro volbu** *(např. spotřeba, determinismus, grafický výkon)*            | Montáž přímo na DIN lištu v rozváděči, integrovaný displej pro nastavení časovačů přímo na místě, robustní reléové výstupy pro motor a semafor, napájení 24 V DC / 230 V AC bez nutnosti vývoje vlastního plošného spoje. | `nizka spotřeba,male rozměry,dostatečny vykon pro měřeni teploty,regualci a jednoduche I/O`| `deterministicke řizeni,robustni prumyslove I/O,snadne řizeni motoru,senzoru a sekveci balici linky`| `vysoka vypočetni a graficky vykon pro zpracovani 4k obrazu,neurovnove sitě a analýza v real time` |
| **Hlavní riziko při volbě špatné platformy** *(proč by neuspěly ostatní dvě varianty)*                | **MCU:** Nutnost vývoje vlastní desky, nízká odolnost vůči venkovnímu rušení a obtížný servis údržbou.<br>**iPC:** Zbytečně extrémní cena (> 30 tis. Kč), dlouhý start po výpadku napájení a vysoká spotřeba.           | `při požiti pc/ipc zbytečne velka cena,rozměry a spotřeba,při přiliš jednoduchem MCU omezene možnosti` | `MCU by vyžadoval rozsahlejší vlastni vyoj a huře by se udržovalo;ipc je zbytečně drahe a energeticky naročné pro běžne řízení linky`| `plc/MCU nemaji dostatečny vypočetni vykon pro naročné zpracovaní obrazu;ipc má vyšší cenu,spořebu a naroky na udržbu` |
...
| :--- | :--- | :--- | :--- |
| **Elektromagnetická kompatibilita (EMC)** | `Arduino je umístěno přímo u hydraulického lisu a ovládá induktivní zátěž (cívky ventilů) přes běžný hobby reléový modul. Chybí doložená ochrana proti přepěťovým špičkám, filtrace a průmyslové EMC řešení` | Napěťové špičky z indukční zátěže hydraulických ventilů způsobí restart MCU... | `Samovolný reset nebo zamrznutí Arduina, falešné sepnutí/vypnutí výstupu, poškození kontaktů relé nebo elektroniky. U řídicího systému lisu může být následkem neočekávaný pohyb hydrauliky nebo přerušení pracovního cyklu.` |
| **Mechanická a teplotní odolnost** | PLA plast a montáž na těleso lisu | `PLA je vhodný především pro běžné prototypování, ale není automaticky vhodným materiálem pro mechanicky a tepelně namáhaný kryt. Přenos tepla z lisu může zvýšit teplotu plastu a způsobit jeho měknutí/deformaci; vibrace a rázy mohou namáhat upevňovací body. Prusa uvádí PLA jako materiál používaný pro široké spektrum běžných 3D tištěných dílů, ale samotný fakt, že je díl vytištěný z PLA, nedokládá vhodnost pro montáž přímo na lis. ` | `PLA je vhodný především pro běžné prototypování, ale není automaticky vhodným materiálem pro mechanicky a tepelně namáhaný kryt. Přenos tepla z lisu může zvýšit teplotu plastu a způsobit jeho měknutí/deformaci; vibrace a rázy mohou namáhat upevňovací body. Prusa uvádí PLA jako materiál používaný pro široké spektrum běžných 3D tištěných dílů, ale samotný fakt, že je díl vytištěný z PLA, nedokládá vhodnost pro montáž přímo na lis. ` |
| **Konektivita a propojení vodičů** | DuPont propojovací kabely bez aretace | `Vibrace mohou způsobit mikropohyby konektoru a postupné uvolnění kontaktu. Prach a mechanické namáhání dále zvyšují pravděpodobnost přerušovaného kontaktu. Přerušení signálu může být pro řídicí systém nebezpečné zejména tehdy, pokud není navržena diagnostika typu „ztráta signálu = bezpečný stav“.` | `Výpadek snímače nebo ovládacího signálu, náhodné resetování či nesprávné ovládání ventilu. Nejhorší varianta je přerušované spojení, které může být obtížně diagnostikovatelné a vést k nepředvídatelnému chování stroje.` |
| **Funkční bezpečnost (Safety)** | Nouzový stop řešený softwarově v čipu | `Pokud Arduino zamrzne, restartuje se, dojde k chybě programu, poruše napájení nebo selhání vstupu D2, software nemusí nouzový stop správně vyhodnotit. Navíc nastavení výstupu na LOW samo o sobě nemusí fyzicky odpojit hydraulickou energii. Arduino Uno sice má externí přerušení na D2, ale to z něj nedělá bezpečnostní obvod. ` | `Nouzový stop nemusí zastavit nebezpečný pohyb. Hydraulický lis může pokračovat v pohybu a obsluha může být vystavena riziku rozdrcení nebo jinému vážnému úrazu. U hydraulických lisů jsou právě pohyblivé části v místě operace významným rizikem; bezpečnostní opatření proto musí být navržena jako ochrana stroje, ne pouze jako běžná funkce programu. ` |
